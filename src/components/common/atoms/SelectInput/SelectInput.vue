...
<script>
  export default {
    ...
    data() {
      return {
        default_options: this.$lget(this.attrs, 'attrs.options', []),
        options: this.$lget(this.attrs, 'attrs.options', []),
      };
    },
    watch: {
      attrs: {
        ...
        deep: true,
        handler(newVal) {
          if (this.default_options.length === 0 || this.default_options.length !== this.$lget(newVal, 'attrs.options', []).length) {
...
<style scoped lang="scss" src="./_SelectInput.scss">
</style>
