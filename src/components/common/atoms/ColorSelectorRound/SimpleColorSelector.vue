    ...
    <q-card flat
      ...
      <q-card-section>
        <div class="row q-mb-5" style="justify-content: center">
          <color-pick v-bind="color.hsla" @update:modelValue="updateColor"></color-pick>
        </div>
      </q-card-section>
    </q-card>
  ...
  export default {
    ...
    watch: {
      color: {
        deep: true,
        handler(newVal) {
          this.updateColor(newVal.hsla.h);
        }
      }
    },
    methods: {
      ...
      updateColor(hue) {
        ...
        let colorVar = Color.hsl(this.color.hsla.h, this.color.hsla.s, this.color.hsla.l).alpha(this.color.hsla.a);
