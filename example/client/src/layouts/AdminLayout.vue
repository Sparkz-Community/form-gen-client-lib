<template>
  <q-layout view="lHh LpR fFf">
    <q-header elevated>
      <q-toolbar>
        <q-btn
               flat
               dense
               round
               icon="menu"
               aria-label="Menu"
               @click="left = !left"/>

        <q-toolbar-title>
          <q-btn flat label="Admin" to="/admin" class="text-h5"/>
          <q-toggle :value="$q.dark.mode"
                    @input="handleDarkModeToggle"
                    checked-icon="fas fa-sun"
                    color="dark"
                    unchecked-icon="fas fa-moon"/>
          <q-btn icon="fas fa-palette" flat @click="themeDio = true"/>
<!--          <q-toggle :value="$q.dark.mode"-->
<!--                    @input="handleDarkModeToggle"-->
<!--                    checked-icon="fas fa-sun"-->
<!--                    color="dark"-->
<!--                    unchecked-icon="fas fa-moon"/>-->
        </q-toolbar-title>

        <q-tabs shrink>
          <q-route-tab v-if="$lisEmpty(user)" name="login" label="Login" :to="{name: 'login'}" exact/>
          <q-route-tab v-if="!$lisEmpty(user)" name="logout" label="Logout" :to="{name: 'logout'}" exact/>
          <q-route-tab v-else name="register" label="Register" :to="{name: 'register'}" exact/>
        </q-tabs>
      </q-toolbar>
    </q-header>
    <q-drawer v-model="left" side="left" bordered>
      <Links :title="title"
             :links-config="links" :dark="$q.dark.mode" :defaultRoute="defaultRoute">
        <template v-slot:user>

          <div class="user">
            <img :src="$lget(user, 'avatar.large.file', 'https://www.w3schools.com/howto/img_avatar.png')" class="img" :style="{width: width, height: width, objectFit: 'cover'}"/>
            <div :style="{color: $q.dark.mode ? '#ffffff' : '#000000'}" class="name">{{ $lget(user, 'name', 'No Name') }}</div>
            <div :style="{color: $q.dark.mode ? '#ffffff' : '#000000'}" class="email">{{ $lget(user, 'email', 'No Email') }}</div>
            <q-icon @click="editProfile" class="icon" name="fas fa-pencil-alt">
              <q-tooltip>Edit profile</q-tooltip>
            </q-icon>
          </div>


        </template>
      </Links>
    </q-drawer>
    <q-page-container>
      <router-view style="width: 100%;"></router-view>
    </q-page-container>

    <q-dialog maximized v-model="themeDio">
      <q-card>
        <q-card-actions align="right" style="position: absolute; right: 0; z-index: 1000">
            <q-btn flat round dense v-close-popup icon="close" class="q-pa-sm" size="md" />
        </q-card-actions>

        <q-card-section class="q-pt-xl">
          <ThemePicker />
        </q-card-section>
      </q-card>
    </q-dialog>
  </q-layout>
</template>

<script>
  import Links from '../components/admin/Links';
  import {mapGetters} from 'vuex';
  import ThemePicker from 'pages/themePicker/themePicker';

  export default {
    name: 'AdminLayout',
    components: {ThemePicker, Links},
    props: {
      sidebarBackground: {
        type: String,
        default: '#ffffff',
      },
      dark: {
        type: Boolean,
        default: false,
      },
      title: {
        type: String,
      },
      sidebarDirection: {
        type: String,
        default: 'left',
      },
      sidebarWidth: {
        type: String,
        default: '19vw',
      },
      linksConfig: {
        type: Array,
        default: () => [],
      },
      defaultRoute: {
        type: Object
      }
    },
    data() {
      return {
        sbWidth: this.sidebarWidth,
        showIcon: false,
        left: true,
        bgImage: 'data:image/png;base64,',
        themeDio: false
      };
    },
    created() {
      let rules = this.$lget(this.user, '_fastjoin.rules', []);
      this.$ability.update(rules);
    },
    computed: {
      ...mapGetters('auth', {
        user: 'user'
      }),
      routesWidth() {
        let newWidth = Number(this.sbWidth.slice(1, this.sbWidth.length));
        return newWidth < 100 ? `.${100 - newWidth}` : String(100 - newWidth);
      },
      width() {
        let num = this.sidebarWidth.replace('vw', '');
        return Math.floor(Number(num) / 2) + 'vw';
      },
      links() {
        return this.linksConfig.map(link => {
          if (this.$lget(link, 'routeTo')) {
            if (link.routeTo.includes('  ')) {
              link.routeTo = link.routeTo.replace(/\s\s+/g, '_');
            } else if (link.routeTo.includes(' ')) {
              link.routeTo = link.routeTo.replaceAll(' ', '_');
            }
            if (link.routeTo.includes(' ')) {
              this.$q.notify({
                message: `${link.routeTo} routeTo cannot contain multiple spaces`,
                color: 'negative',
              });
            }
          }
          return link;
        });
      },
    },
    methods: {
      editProfile() {
        if (this.$route.name !== 'my-profile') {
          this.$router.push({path: '/admin/my-profile'});
        }
      },
      handleDarkModeToggle(val) {
        if (typeof val === 'boolean') {
          this.$q.dark.set(val);
          if (this.user) {
            let payload = {
              'theme.darkMode': val,
            };
            this.user.patch({data: payload})
              .then(res => {
                this.$q.notify({
                  type: 'positive',
                  message: `Successfully Updated "${res.email}" Dark Mode theme`,
                  timeout: 10000,
                  actions: [
                    {
                      icon: 'close', color: 'white', handler: () => {
                        /* ... */
                      },
                    },
                  ],
                });
              })
              .catch(err => {
                this.$q.notify({
                  type: 'negative',
                  message: err.message,
                  timeout: 10000,
                  actions: [
                    {
                      icon: 'close', color: 'white', handler: () => {
                        /* ... */
                      },
                    },
                  ],
                });
              });
          }
        }
      },
    }
  };
</script>

<style lang="scss">
  @import url('https://fonts.googleapis.com/css2?family=Dela+Gothic+One&display=swap');
  @import url('https://fonts.googleapis.com/css2?family=Source+Sans+Pro:wght@400;600;900&display=swap');

  .sourceSansProBold {
    font-family: 'Source Sans Pro', sans-serif;
  }
  .user {
    display: flex;
    align-items: center;
    flex-direction: column;
    justify-content: center;
    padding: 30px 0 20px 0;

    .img {
      transition: 0.2s all;
      border-radius: 50%;
      position: relative;
    }

    .icon {
      position: absolute;
      right: 20px;
      top: 20px;
      color: blue;
      transition: 0.1s all;
      font-size: 1.4em;
      cursor: pointer;
    }

    .img:hover {
      cursor: pointer;
    }

    .img:hover .icon {
      color: #00aaf1;
    }

    .name {
      margin-top: 10px;
      margin-bottom: 5px;
      font-size: 1.2em;
      font-weight: 500;
      text-align: center;
    }
    .email {
      text-align: center;
    }
  }

  .main-container {
    width: 100vw;
    height: 100vh;
    display: flex;
    align-items: flex-start;
  }

  .header {
    display: flex;
    align-items: center;
    justify-content: space-between;
    color: white;
    background-color: #24252A;
    padding: 15px 100px;


    .left {
      font-family: 'Dela Gothic One', sans-serif;
      font-size: 1.1em;
    }

    .center {
      display: flex;
      align-items: center;
      justify-content: space-between;

      span {
        margin: 0 20px;
        color: #cccfd4;
      }
    }

    .right .btn {
      background-color: rgb(0, 136, 168);
      border-radius: 15px;
      padding: 5px 10px;
      cursor: pointer;
    }
  }

  #links {
    transition: 0.4s all;
  }

  @keyframes hideMenu {
    from {
      margin-left: 0
    }
    to {
      margin-left: -1000px
    }
  }

  .hidden-menu {
    animation: hideMenu 0.5s;
  }

  .menu-btn {
    position: absolute;
    top: 10px;
    left: 15px;
    color: white;
    font-size: 1.6em;
    cursor: pointer;
  }
</style>
